;(function (factory) {
  ...
  if (typeof define === 'function' && define.amd) {
    ...
    module.exports = factory(root, module.exports, root.Rx ? root.Rx : require('rx'));
  } else {
  ...
  observableProto.mapTemplate = function(template_code) {
    return this.map(_template(template_code));
  }
  ...
  observableProto.objectMerge = function(obj, obj2) {
    return this.map(function(x) { _.defualtsDeep(obj, x, obj2) });
  }
  ...
  observableProto.subscribeAlt = function(cb) {
    this.subscribe(function(data, err) {
      cb(err, data);
    });
  }
  ...
  observableProto.propertyFilter = function (props, filterBy) {
    ...
    else for(var i = 0; i < len; i++) { args[i] = arguments[i]; }
